<template>
<div class="container p-3">
    <div class="row">
        <div class="col-8">
            <div class="row flex-wrap gap-4 justify-center">
                <div class="col-5 border   py-2 rounded"  v-for="(post, index) in posts" :key="index">
                    <div class="card">
                        <img class="rounded" src="data:image/jpeg;base64," alt="">
                    </div>
                    <div class="card-body">
                        <h5 class="card-title" >{{ post.title }}</h5>
                        <small class="text-muted">Last updated {{ post.created_at }}</small>
                        <p class="card-text text-wrap">{{ post.description }}</p>
                    </div>
                    <div class="card-footer d-flex justify-content-end gap-3 bg-transparent border-top-0">
                        <div class="btn btn-sm btn-warning" @click="editItem(post)" >Edit</div>
                        <div class="btn btn-sm btn-danger" @click="deleteItem(post)" >Delete</div>
                    </div>
                </div>
            </div>
        </div>
        <div class="row col-4">
            <div >
                <div class="card-body border p-2 rounded-md">
                    <form method="POST" @submit.prevent="saveItem()">
                        <div class="form-group">
                            <label for="title">Title</label>
                            <input type="text" class="form-control" v-model="title" placeholder="Enter title" required>
                        </div>
                        <div class="form-group">
                            <label for="title">Description</label>
                            <textarea name="" class="form-control" v-model="description" placeholder="Enter description" id="" cols="30" rows="10" required></textarea>
                        </div>
                        <div class="form-group mt-2 text-end">
                            <button class="btn btn-primary m">
                                Submit
                            </button>
                        </div>

                    </form>
                </div>
            </div>
        </div>
    </div>

</div>
</template>

<script setup>
import { storeToRefs } from 'pinia';
import { postStore } from '../store/post/postStor';
import { onMounted } from 'vue';
const { posts, title, description } = storeToRefs(postStore())
const { saveItem, getPosts, editItem, deleteItem } = postStore();

onMounted(() => {
    getPosts();
})
</script>


